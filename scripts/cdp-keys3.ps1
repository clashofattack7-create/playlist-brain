...
try {
  ...
  for ($idx = 0; $idx -lt 4; $idx++) {
    $expr = "(() => { const v = Array.from(document.querySelectorAll('input[type=number]')).filter(x => x.offsetParent !== null && ['first','second','third','fourth'].includes(x.id)); if (v[" + $idx + "]) { v[" + $idx + "].value = ''; v[" + $idx + "].focus(); } return v.length; })()"
    [void](Send-Cmd 'Runtime.evaluate' @{ expression = $expr; returnByValue = $true })
    Start-Sleep -Milliseconds 350
    [void](Send-Cmd 'Input.insertText' @{ text = $digits[$idx] })
    Start-Sleep -Milliseconds 500
  }
  $r1 = Send-Cmd 'Runtime.evaluate' @{ expression = "JSON.stringify(Array.from(document.querySelectorAll('input[type=number]')).filter(i=>i.offsetParent!==null && ['first','second','third','fourth'].includes(i.id)).map(i=>i.value))"; returnByValue = $true }
